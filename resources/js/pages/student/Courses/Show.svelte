<script lang="ts">
  ...
  import { Undo2, BookOpen, Eye, ClipboardList } from 'lucide-svelte';
...
<StudentLayout {breadcrumbs}>
  <div class="container mx-auto px-6 py-8">
    ...
    <!-- Card para ver actividades -->
    <Card.Root class="mb-8 bg-gradient-to-r from-violet-50 to-purple-50 border-violet-200">
      <Card.Content class="p-8">
        <div class="flex items-center justify-between">
          <div class="flex items-center gap-4">
            <div class="p-3 bg-violet-100 rounded-lg">
              <ClipboardList class="text-violet-600" size={28} />
            </div>
            <div>
              <h3 class="text-lg font-bold text-slate-900">Actividades del Curso</h3>
              <p class="text-sm text-slate-600">Consulta tus tareas, evaluaciones y sus notas</p>
            </div>
          </div>
          <Link
            href={`/estudiante/cursos/${curso.id_curso}/actividades`}
            class="inline-flex items-center gap-2 px-6 py-3 bg-violet-600 hover:bg-violet-700 text-white font-medium rounded-lg transition-colors"
          >
