<script lang="ts">
	...
	async function handleSubmit(e: SubmitEvent) {
		...
		try {
			await router.put(
				inscripciones_cursos.update(`${inscripcion.id_curso},${inscripcion.id_estudiante}`).url,
				formData
			);
		} catch (error) {
			if (error instanceof AxiosError) {
				if (error.response?.status === 422 && error.response?.data?.errors) {
					...
					errorMessage = error.response?.data?.message || 'Error al actualizar la inscripción';
				}
			} else {
				errorMessage = 'Error al actualizar la inscripción';
			}
			isSubmitting = false;
		}
	}
	...
	function handleDelete() {
		if (confirm('¿Estás seguro de que deseas eliminar esta inscripción? Esta acción no se puede deshacer.')) {
			router.delete(
				inscripciones_cursos.destroy(`${inscripcion.id_curso},${inscripcion.id_estudiante}`).url
			);
		}
	}
</script>
				...
				<form onsubmit={handleSubmit} class="space-y-6">
					...
					<div>
						...
						{#if errors.promedio_parcial}
							<p class="mt-2 text-sm text-red-600">{errors.promedio_parcial[0]}</p>
						{/if}
					</div>
