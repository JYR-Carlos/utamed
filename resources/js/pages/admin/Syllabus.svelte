        ...
        <table class="w-full text-sm">
          ...
          <tbody>
            ...
            {#each cursos as curso (curso.id_curso)}
              ...
              {@const badge  = curso.programa ? (estadoBadge[curso.programa.estado] ?? { label: curso.programa.estado, cls: 'bg-slate-100 text-slate-700' }) : null}
              ...
              <tr class="border-b border-slate-100 hover:bg-slate-50 transition-colors">
                ...
                <td class="py-3 px-4 text-center">
                  {#if accion.tipo === 'dias-basico'}
                    ...
                    <div
                      class="inline-flex flex-col items-center justify-center rounded-lg px-3 py-1.5 text-center leading-tight
                        {d < 0 ? 'bg-red-50 text-red-700 border border-red-200' : d <= 3 ? 'bg-orange-50 text-orange-700 border border-orange-200' : 'bg-blue-50 text-blue-700 border border-blue-200'}"
                    >
                  ...
                  {:else if accion.tipo === 'dias-completo'}
                    ...
                    <div
                      class="inline-flex flex-col items-center justify-center rounded-lg px-3 py-1.5 text-center leading-tight
                        {d < 0 ? 'bg-red-50 text-red-700 border border-red-200' : d <= 3 ? 'bg-orange-50 text-orange-700 border border-orange-200' : 'bg-indigo-50 text-indigo-700 border border-indigo-200'}"
                    >
                      <span class="text-lg font-bold">{d < 0 ? 0 : d}</span>
                      <span class="text-[10px] font-medium">
...
<Dialog.Root
  open={dialog.open}
  onOpenChange={(v) => { if (!v) cerrarDialog(); }}
>
  <Dialog.Content class="max-w-md">
    ...
    <div class="space-y-5 py-2">
      ...
      <div class="space-y-1.5">
        ...
        <Input
          id="fecha-completo"
          type="date"
          bind:value={dialog.fechaCompleto}
          min={dialog.fechaBasico || undefined}
          class="text-sm"
        />
        <p class="text-xs text-slate-400">
    ...
    <Dialog.Footer class="gap-2">
      <Button variant="ghost" onclick={cerrarDialog} disabled={dialog.saving}>
        Cancelar
      </Button>
      <Button
