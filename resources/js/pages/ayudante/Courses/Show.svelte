<script lang="ts">
	...
	const breadcrumbs: BreadcrumbItem[] = [
		...
		{ label: curso?.nombre || `Curso ${id_curso}`, href: '' }
	];
</script>

<AyudanteLayout title={curso?.nombre || `Curso ${id_curso}`} {breadcrumbs}>
	<div class="space-y-6">
		<!-- Course Header Card -->
				...
				<Card.Root class="bg-yellow-50 border-yellow-200">
					<Card.Content class="pt-6">
						<p class="text-sm text-yellow-800">
							No hay programa disponible para este curso en este momento.
						</p>
					</Card.Content>
				</Card.Root>
			{/if}
