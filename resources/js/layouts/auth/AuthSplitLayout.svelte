...
<div class="flex min-h-screen bg-background text-foreground selection:bg-primary/30">
  <!-- Left Column: Login Form -->
  <div
    class="flex flex-1 flex-col justify-center px-4 py-4 sm:px-12 md:px-12 lg:flex-none lg:w-[450px] xl:w-[500px] z-10 bg-background"
  >
    <div class="mx-auto w-full max-w-sm lg:w-96" in:fly={{ y: 20, duration: 600, delay: 200 }}>
      ...
      <div class="mt-12 flex items-center justify-between text-xs text-muted-foreground">
  ...
  <!-- Right Column: Illustration (Dynamic Design) -->
  <div
    class="hidden lg:relative lg:flex lg:flex-1 lg:items-center lg:justify-center bg-primary/5 overflow-hidden border-l border-border"
  >
    <!-- Abstract Shapes / Blobs -->
    <div class="absolute inset-0 z-0">
      <div
        class="absolute -top-24 -left-24 h-96 w-96 rounded-full bg-primary/10 opacity-20 blur-3xl animate-pulse"
      ></div>
      <div
        class="absolute -bottom-24 -right-24 h-[500px] w-[500px] rounded-full bg-primary/20 opacity-20 blur-3xl"
      ></div>
      <div
        class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 h-[800px] w-[800px] rounded-full bg-primary/5 opacity-10 blur-3xl"
      ></div>
    </div>

    <div
      class="relative z-10 flex flex-col items-center text-center p-12 max-w-2xl"
      in:fade={{ duration: 1000 }}
    >
      <div class="space-y-6 mb-12">
        <h2 class="text-5xl font-extrabold tracking-tight text-foreground leading-tight">
          Bienvenido al <br />
    ...
    <div
      class="absolute inset-x-0 bottom-0 h-32 bg-gradient-to-t from-background/50 to-transparent pointer-events-none"
    ></div>
  </div>
</div>
