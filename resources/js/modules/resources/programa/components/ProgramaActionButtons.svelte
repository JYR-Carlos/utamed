<script lang="ts">
  ...
  let { programa, userRole, userId, onApprove, onReject, onEdit }: Props = $props();
  ...
  async function handleApprove() {
    isSubmitting = true;
    try {
      await fetch(`/admin/programas/${programa.id_programa}/approve`, {
        method: 'POST',
        headers: {
  ...
  async function handleReject() {
    ...
    isSubmitting = true;
    try {
      await fetch(`/admin/programas/${programa.id_programa}/reject`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]')?.getAttribute('content') || '',
        },
        body: JSON.stringify({ razon_rechazo: rejectReason }),
      });
...
<div class="flex flex-wrap items-center gap-2">
  ...
  <!-- Editar -->
  {#if canEdit}
    <Button variant="outline" size="sm" onclick={() => onEdit(programa.id_programa)} title="Editar programa">
      <Edit2 size={16} />
    </Button>
  {/if}
  ...
  <!-- Rechazar (Solo Admin) -->
  {#if canReject}
    <Button variant="destructive" size="sm" onclick={() => (showRejectDialog = true)} title="Rechazar este programa">
    ...
    <div class="bg-white rounded-lg shadow-xl max-w-md w-full">
      ...
      <div class="px-6 py-4">
        ...
        <p class="text-sm text-slate-600 mb-4">
          El estado cambiará de <code class="bg-gray-100 px-1 rounded">COMPLETO</code> a <code class="bg-gray-100 px-1 rounded">APROBADO</code>.
        </p>
      ...
      <div class="bg-gray-50 px-6 py-4 border-t flex justify-end gap-3">
        <Button variant="outline" onclick={() => (showApproveDialog = false)} disabled={isSubmitting}>Cancelar</Button>
        <Button variant="default" class="bg-green-600 hover:bg-green-700" onclick={handleApprove} disabled={isSubmitting}>
  ...
  <div class="fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4">
    <div class="bg-white rounded-lg shadow-xl max-w-md w-full">
      ...
      <div class="px-6 py-4">
        ...
        <div class="mt-4 bg-yellow-50 p-3 rounded-lg text-sm text-yellow-800 border border-yellow-200">
          <strong>Nota:</strong> El estado volverá a <code class="bg-yellow-100 px-1 rounded">BASICO_COMPLETO</code> o
      ...
      <div class="bg-gray-50 px-6 py-4 border-t flex justify-end gap-3">
        <Button variant="outline" onclick={() => (showRejectDialog = false)} disabled={isSubmitting}>Cancelar</Button>
