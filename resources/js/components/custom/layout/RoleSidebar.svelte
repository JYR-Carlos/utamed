<script lang="ts">
    ...
    import {
        ...
        Search, Settings, Calendar, GraduationCap, ClipboardList,
        MessageSquare, BookOpenCheck, LayoutGrid, Users, Building2
    } from 'lucide-svelte';
    ...
    let filteredDocente = $derived(
        docenteCourses.filter(c =>
            !searchQuery || c.nombre.toLowerCase().includes(searchQuery.toLowerCase())
        )
    );
    let filteredEstudiante = $derived(
        estudianteCourses.filter(c =>
            !searchQuery || c.nombre.toLowerCase().includes(searchQuery.toLowerCase())
        )
    );
    let filteredAyudante = $derived(
        ayudanteCourses.filter(c =>
            !searchQuery || c.nombre.toLowerCase().includes(searchQuery.toLowerCase())
        )
    );
</script>
...
<div class="h-full flex flex-col bg-white text-slate-700">
    ...
    <div class="flex-1 overflow-y-auto py-6">
        ...
        {#if isDocente}
            ...
            <!-- Root link -->
            <div class="px-4 mb-2">
                <Link href="/docente/cursos" class="flex items-center gap-3 px-4 py-2.5 rounded-xl text-[15px] font-bold text-slate-800 hover:bg-slate-50 hover:text-indigo-600 transition-all group">
            ...
            <div class="px-4 flex flex-col gap-1">
                            ...
                            {#if expanded}
                                <div class="pl-11 pr-2 py-1 flex flex-col gap-1 mb-2">
                                    ...
                                    {#if curso.tiene_programa}
                                        <Link href="/docente/cursos/{curso.id_curso}/programa" class="flex items-center justify-between px-3 py-2 rounded-lg text-sm font-medium text-slate-600 hover:bg-white hover:shadow-sm hover:text-indigo-600 transition-all">
                                    ...
                                    {/if}
                                    <!-- Actividades -->
                                    <Link href="/docente/cursos/{curso.id_curso}/actividades" class="flex items-center gap-2 px-3 py-2 rounded-lg text-sm font-medium text-slate-600 hover:bg-white hover:shadow-sm hover:text-indigo-600 transition-all">
                ...
                <!-- Footer style links -->
                 <Link href="/docente/dashboard" class="flex items-center gap-3 px-4 py-2.5 rounded-xl text-[14px] font-semibold text-slate-600 hover:bg-slate-50 hover:text-indigo-600 transition-all group">
                    <LayoutGrid size={18} class="text-slate-400 group-hover:text-indigo-500 transition-colors" />
                    Dashboard
                </Link>
                <Link href="/docente/inscripciones" class="flex items-center gap-3 px-4 py-2.5 rounded-xl text-[14px] font-semibold text-slate-600 hover:bg-slate-50 hover:text-indigo-600 transition-all group">
                    <Users size={18} class="text-slate-400 group-hover:text-indigo-500 transition-colors" />
                    Inscripciones
                </Link>
                <Link href="/settings" class="flex items-center gap-3 px-4 py-2.5 rounded-xl text-[14px] font-semibold text-slate-600 hover:bg-slate-50 hover:text-indigo-600 transition-all group">
        ...
        {#if isEstudiante && !isDocente}
            ...
            <div class="px-4 mb-2">
                <Link href="/estudiante/dashboard" class="flex items-center gap-3 px-4 py-2.5 rounded-xl text-[15px] font-bold text-slate-800 hover:bg-slate-50 hover:text-indigo-600 transition-all group">
            ...
            <div class="px-4 flex flex-col gap-1">
                ...
                {:else}
                    {#each filteredEstudiante as curso (curso.id_curso)}
                        <Link href="/estudiante/cursos/{curso.id_curso}" class="flex items-center gap-3 px-4 py-2.5 rounded-xl text-[14px] font-semibold text-slate-600 hover:bg-slate-50 hover:text-indigo-600 transition-all group">
                ...
                <Link href="/estudiante/dashboard" class="flex items-center gap-3 px-4 py-2.5 rounded-xl text-[14px] font-semibold text-slate-600 hover:bg-slate-50 hover:text-indigo-600 transition-all group">
                    <LayoutGrid size={18} class="text-slate-400 group-hover:text-indigo-500 transition-colors" />
                    Dashboard
                </Link>
                <Link href="/settings" class="flex items-center gap-3 px-4 py-2.5 rounded-xl text-[14px] font-semibold text-slate-600 hover:bg-slate-50 hover:text-indigo-600 transition-all group">
        ...
        {#if isAyudante && !isDocente}
             ...
             <div class="px-4 mb-2">
                <Link href="/ayudante/dashboard" class="flex items-center gap-3 px-4 py-2.5 rounded-xl text-[15px] font-bold text-slate-800 hover:bg-slate-50 hover:text-indigo-600 transition-all group">
             ...
             <div class="px-4 flex flex-col gap-1">
                ...
                {/if}
                 <div class="h-px bg-slate-100 my-4 mx-4"></div>
                <Link href="/ayudante/dashboard" class="flex items-center gap-3 px-4 py-2.5 rounded-xl text-[14px] font-semibold text-slate-600 hover:bg-slate-50 hover:text-indigo-600 transition-all group">
                    <LayoutGrid size={18} class="text-slate-400 group-hover:text-indigo-500 transition-colors" />
                    Dashboard
                </Link>
                <Link href="/settings" class="flex items-center gap-3 px-4 py-2.5 rounded-xl text-[14px] font-semibold text-slate-600 hover:bg-slate-50 hover:text-indigo-600 transition-all group">
        ...
        {#if isAdmin}
            <div class="px-6 mb-2">
             ...
             <div class="px-4 mb-2">
                <Link href="/dashboard" class="flex items-center gap-3 px-4 py-2.5 rounded-xl text-[15px] font-bold text-slate-800 hover:bg-slate-50 hover:text-indigo-600 transition-all group">
            ...
            <div class="px-4 flex flex-col gap-1">
                {#each [
                    ...
                    <Link href={item.href} class="flex items-center gap-3 px-4 py-2.5 rounded-xl text-[14px] font-semibold text-slate-600 hover:bg-slate-50 hover:text-indigo-600 transition-all group">
