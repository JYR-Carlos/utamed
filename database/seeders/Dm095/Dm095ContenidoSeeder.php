...
/**
 * Carga el CONTENIDO de DM095 (Taller Profesional IV) en cursos que ya existen:
 * unidades, programa, bibliografía y actividades, para cada paralelo listado en
 * la hoja Cursos de programa_dm095.xlsx.
 *
 * Pensado para producción, donde el wizard del admin e Intranet ya crearon los
 * cursos A y B con sus componentes e inscripciones (foto en
 * estado_produccion_2026-09-21.xlsx). Por eso NO crea cursos, componentes ni
 * inscripciones: si el curso no está, lo dice y sigue con el siguiente.
 *
 ...
 *  - rúbricas: las actividades que tienen matriz de evaluación (un libro
 *    matriz_evaluacion_*.xlsx) reciben su fila en agenda.rubrica, salvo que la
 *    que ya tengan esté cerrada o con evaluaciones hechas.
 *
 * Opciones por entorno:
 *  - DM095_BIBLIO_URL_PLACEHOLDER: base de URL para los recursos que vienen sin
 *    url ni archivo. Sin ella, esas filas se OMITEN (la tabla exige url o archivo
 *    cuando es_bibliografia_uta = false) y quedan listadas al final.
 *  - DM095_PROGRAMA_ESTADO: estado del programa cuando hay que crearlo
 *    (default: el del hermano, o APROBADO).
 */
class Dm095ContenidoSeeder extends Seeder
{
    /** Escribir en la sección VIII aunque el programa esté APROBADO/PUBLICADO. */
    private const ACTUALIZAR_PROGRAMAS_APROBADOS = true;

    /** Al clonar el programa del hermano, poner I.codigo = cod_asignatura (producción tiene el cod_curso). */
    ...
    public function run(): void
    {
        ...
        $this->command->info('== Dm095ContenidoSeeder: unidades, programa, bibliografía y actividades ==');
        ...
        if ($this->omitidas) {
            $this->command->warn("\nBibliografías OMITIDAS por no tener url ni archivo (define DM095_BIBLIO_URL_PLACEHOLDER o completa la columna url del Excel):");
    ...
    private function sembrarCurso(Curso $curso, array $fila): void
    {
        ...
        $tipoComponente = $this->datos->actividades()[0]['componente_tipo'] ?? 'Taller';
        $componente = Componente::where('id_curso', $curso->id_curso)
            ->whereHas('tipoComponente', fn ($q) => $q->where('tipo', $tipoComponente))
            ->first();
        if (!$componente) {
            throw new \RuntimeException("El curso {$curso->id_curso} no tiene componente «{$tipoComponente}»; los componentes los crea Intranet, no este seeder.");
        }
        ...
        $this->sembrarActividades($curso, $componente, $unidades);
    }

    /** Producción tenía Taller#12 (B) sin docente_componente: se le pone el titular. */
    private function asegurarDocenteComponente(Componente $componente, Docente $titular): void
    {
        $existe = DocenteComponente::where('id_componente', $componente->id_componente)->exists();
        if ($existe) {
            return;
        }
    ...
    private function asegurarPrograma(Curso $curso, int $idUsuario): Programa
    {
        ...
        $estado = env('DM095_PROGRAMA_ESTADO') ?: ($hermano?->estado ?? 'APROBADO');
    ...
    {
        $placeholder = env('DM095_BIBLIO_URL_PLACEHOLDER');
        $entradasJson = [];
    ...
    /** @param array<int, Unidad> $unidades */
    private function sembrarActividades(Curso $curso, Componente $componente, array $unidades): void
    {
        foreach ($this->datos->actividades() as $a) {
            ...
            if ($existente) {
                ...
                $this->command->line("   actividad «{$a['nombre']}»: creada (id {$actividad->id_actividad}).");
            }
