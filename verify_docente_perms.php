...
/**
 * Script para verificar permisos del rol Docente en la BD
 * 
 * Uso: php verify_docente_perms.php
 */
...
$permisosAsignados = $docenteRole->permisos()->count();
echo "Rol: Docente (ID: {$docenteRole->id_rol})\n";
...
$permisos = $docenteRole->permisos()
    ->orderBy('slug')
    ->select('id_permiso', 'slug', 'descripcion')
    ->get();
...
$permisosPrograma = $docenteRole->permisos()
    ->where('slug', 'like', '%programa%')
    ->orderBy('slug')
    ->select('slug', 'descripcion')
    ->get();

if ($permisosPrograma->count() == 0) {
...
$tienePermisoVer = $docenteRole->permisos()
    ->where('slug', 'cursos/programas:ver')
    ->exists();
...
if (count($faltantes) == 0 && $tienePermisoVer) {
    ...
    echo "    Y luego ejecutar el SQL generado en la BD\n";
}
